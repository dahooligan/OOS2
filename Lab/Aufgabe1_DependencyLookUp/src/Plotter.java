...
public class Plotter extends JPanel {
	...
	// Verschiebung der Punkte und Linien, zwecks Uebersichtlichkeit
	...
	public Plotter(int length, int height) {
		...
		// Die Groesse von Frame ist die absolute Groesse des Fensters
		// Daher wird durch Offsets Platz geschaffen, um
		// jeden Punkt ohne Verschiebung anzeigen zu koennen
	...
	public void plot() {
		...
		Object component = Registratur.getRegistratur().getKomponente("Datenquelle");

		// Wenn keine Datenquelle in der Registratur vorhanden ist,
		// beende den Plotvorgang
		if (!(component instanceof IDatenquelle)) {
			return;
		}
		
		datenquelle = (IDatenquelle) component;
	...
	protected void paintComponent(Graphics g) {
		...
		Point minPunkte = new Point(Collections.min(xPunkte),
				Collections.min(yPunkte));
		Point maxPunkte = new Point(Collections.max(xPunkte),
				Collections.max(yPunkte));

		// Offset errechnen, um Position der Punkte auf das Fenster anzupassen
		Point offset = new Point(plotterLength, plotterHeight);
		// Differenz der Min- und Max-Werte der x- und y-Wertreihe pruefen,
		...
		for (int i = 0; i < datenreihe.size(); i++) {
			...
			if (i < (datenreihe.size() - 1)) {
				...
				g.drawLine(
						(Math.abs(minPunkte.x) + xPunkte.get(i)) * offset.x
								+ xVerschiebung,
						plotterHeight
								- (Math.abs(minPunkte.y) + yPunkte.get(i))
								* offset.y + yVerschiebung,
						(Math.abs(minPunkte.x) + xPunkte.get(i + 1)) * offset.x
								+ xVerschiebung,
						plotterHeight
								- (Math.abs(minPunkte.y) + yPunkte.get(i + 1))
								* offset.y + yVerschiebung);
			}
			...
			g.drawLine(0,
					plotterHeight - (Math.abs(minPunkte.y) + yPunkte.get(i))
							* offset.y + yVerschiebung, 1, plotterHeight
							- (Math.abs(minPunkte.y) + yPunkte.get(i))
							* offset.y + yVerschiebung);
			g.drawString(
					"" + yPunkte.get(i),
					...
					plotterHeight
							+ 5
							- (Math.abs(Collections.min(yPunkte)) + yPunkte
									.get(i)) * offset.y + yVerschiebung);

			// Beschriftung der x-Achsenpunkte einzeichnen
			g.drawLine((Math.abs(minPunkte.x) + xPunkte.get(i)) * offset.x
					+ xVerschiebung, plotterHeight + 25,
					(Math.abs(minPunkte.x) + xPunkte.get(i)) * offset.x
							+ xVerschiebung, plotterHeight + 20);
			g.drawString("" + xPunkte.get(i),
					(Math.abs(minPunkte.x) + xPunkte.get(i)) * offset.x
							+ xVerschiebung, plotterHeight + 20);

			// Punkt markieren
			g.drawOval((Math.abs(minPunkte.x) + xPunkte.get(i)) * offset.x
					+ xVerschiebung - 3, plotterHeight
					- (Math.abs(minPunkte.y) + yPunkte.get(i)) * offset.y
					+ yVerschiebung - 3, 6, 6);
		}
	}
}
