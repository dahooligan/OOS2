...
Mehrere Threads sollen miteinander kommunizieren können und für den
...
\begin{frame}
\begin{block}{Problem}
Producer und Consumer dürfen nicht gleichzeitig auf ein Element zugreifen
\end{block}
\end{frame}

\section{die Lösung}
\subsection{kritische Abschnitte schützen}
\begin{frame}
\begin{block}{kritische Abschnitte schützen}
...
\pause
   \begin{itemize}
     \item<1-> zu schützendes Element wird im Monitor verborgen
...
\begin{itemize}
  ...
  \item Konsistenz der Daten ist Gewährleistet
\end{itemize}
\end{block}
\end{column}
\begin{column}{.5\textwidth}
...
\setbeamertemplate{itemize items}[square]
\begin{itemize}
  \item Unbekannte Ausführungsreihenfolge bei Freigabe von Element
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
\begin{column}{.5\textwidth}
\centering
\includegraphics[width=.9\textwidth]{img/pipesandfilter.png}
\end{column}
\end{columns}
\end{frame}
...
\item zugreifende Objekte müssen warten bis das Objekt in diesem Zustand ist
\end{itemize}
\end{block}
...
% \pause
% \item Plattformunabhängig (passende JVM vorausgesetzt)
% \pause
...
% \begin{column}{.5\textwidth}
% \begin{itemize}
%   \item Unterstützt über 18 Dateiformate
%   \item kompatibel zu ImageIO-Plug-Ins
%   \item Effizient auch bei großen Dokumenten
%   \item Revisionssicheres Einfügen von Annotationen
% \end{itemize}
% 
% \end{column}
% \begin{column}{.5\textwidth}
...
% \begin{column}{.5\textwidth}
% \begin{itemize}
%   \item Benötigt clientseitig keine JVM
%   \item Läuft komplett im Browser ohne weitere Plug-Ins
...
%   \pause
%   \item $\frac{1}{10}$ bis $\frac{1}{3}$ so groß wie XML\footnotemark
%   \item 20-100 mal schneller\footnotemark[1]
%   \item Veröffentlicht unter BSD-Lizenz
% \end{itemize}
% \end{column}
% \pause
% \begin{column}{.5\textwidth}
...
%   \item Einfach einzubinden
% \pause
%   \item Veröffentlicht unter Apache 2.0-Lizenz
% \end{itemize}
% \end{frame}
% % 
% % \begin{frame}
...
% %   \pause
% %   \item {Getrennte Quellen für Dokumente und eingebettete Bilder oder
% %   Schriftarten}
% %   \pause
% % \end{itemize}
% % \end{frame}
...
% % 
% % \begin{frame}
% % \frametitle{Digitale Inklusion - technische Lösungen zur Inklusion in der
% % digitalen Welt}
% % \begin{itemize}
% %   \item {Rechtliche Aspekte (UNO-Behindertenrechtskonvention /
% % Gesetzgebung in Deutschland / rechtlicher Rahmen in anderen Ländern)} 
...
% % \end{itemize}
% % 
% % \item{technische Lösungen (Screenreader, Braille-Zeilen)}
